...
const PUBLIC_COMMANDS = new Set(["release", "teamswap", "roster", "teamcreate", "teamdisband", "teamlist", "overroster", "managerswap", "fofill", "promote", "demote", "demand", "threadlock"]);
...
const commandList = [offer.command, release.command, teamswap.command, roster.command, teamcreate.command, teamdisband.command, teamlist.command, overroster.command, managerswap.command, teamstaff.fofillCommand, teamstaff.promoteCommand, teamstaff.demoteCommand, demand.command, applications.command, moderation.command, threadlock.command, config.command, robloxverify.command, ticketclose.command, invites.command, afkCommand.command];
...
client.on("interactionCreate", async interaction => {
  ...
  if (interaction.isButton() || interaction.isStringSelectMenu() || interaction.isRoleSelectMenu?.() || interaction.isChannelSelectMenu?.() || interaction.isUserSelectMenu?.() || interaction.isModalSubmit()) {
    ...
    if (interaction.customId?.startsWith("application_")) { try { if (interaction.customId === "application_select") await applications.handleApplicationSelect(interaction); else if (interaction.customId.startsWith("application_accept:") || interaction.customId.startsWith("application_reject:")) await applications.handleApplicationReview(interaction); else await applications.handleApplicantInteraction(interaction); } catch (error) { if (!isUnknownInteraction(error)) console.error("Application interaction error:", error); await safeInteractionError(interaction, "Something went wrong while handling that application."); } return; }
  }
  ...
  if (interaction.isModalSubmit()) { try { if (interaction.customId.startsWith("offer_confirm:")) { await offer.handleOfferModal(interaction); return; } } catch (error) { if (!isUnknownInteraction(error)) console.error(error); await safeInteractionError(interaction, "Something went wrong while confirming that offer."); } }
});
client.on("messageCreate", async message => { try { await applications.handleApplicationDM(message); } catch (error) { console.error("Application DM error:", error); } try { await afk.handleMessage(message); } catch (error) { console.error("AFK handler error:", error); } });
...
client.on("guildCreate", async guild => { try { await inviteTracker.refreshGuild(guild); } catch (error) { console.error(`Invite tracking initialization error for ${guild.name}:`, error); } });
client.on("guildMemberUpdate", async (_oldMember, newMember) => { const database = loadData(); const managerrole = require("./commands/managerrole.js"), assistantmanagerrole = require("./commands/assistantmanagerrole.js"), playermanagerrole = require("./commands/playermanagerrole.js"); if (managerrole.isManagerInGuild(database, newMember.guild, newMember.id)) await managerrole.syncManagerMemberRoles(newMember, database, "Restoring required manager and team roles").catch(console.error); if (assistantmanagerrole.isAssistantManagerInGuild(database, newMember.guild, newMember.id)) await assistantmanagerrole.syncAssistantManagerMemberRoles(newMember, database, "Restoring required assistant manager and team roles").catch(console.error); if (playermanagerrole.isPlayerManagerInGuild(database, newMember.guild, newMember.id)) await playermanagerrole.syncPlayerManagerMemberRoles(newMember, database, "Restoring required player manager and team roles").catch(console.error); });
client.once("clientReady", async readyClient => { console.log(`${readyClient.user.tag} is online`); const managerrole = require("./commands/managerrole.js"), assistantmanagerrole = require("./commands/assistantmanagerrole.js"), playermanagerrole = require("./commands/playermanagerrole.js"); await managerrole.syncAllManagerRoles(readyClient); await assistantmanagerrole.syncAllAssistantManagerRoles(readyClient); await playermanagerrole.syncAllPlayerManagerRoles(readyClient); await inviteTracker.initialize(readyClient); if (process.env.ROBLOX_CLIENT_ID && process.env.ROBLOX_CLIENT_SECRET && process.env.ROBLOX_REDIRECT_URI && process.env.ROBLOX_GUILD_ID && process.env.ROBLOX_VERIFIED_ROLE_ID) robloxverify.startWebServer(readyClient); else console.warn("Roblox verification is disabled: missing one or more ROBLOX_* environment variables."); const rest = new REST({ version: "10" }).setToken(token); try { console.log("Refreshing slash commands..."); const guildIds = [...readyClient.guilds.cache.keys()]; for (let i = 0; i < guildIds.length; i++) { await rest.put(Routes.applicationGuildCommands(readyClient.user.id, guildIds[i]), { body: [] }); if (i < guildIds.length - 1) await new Promise(resolve => setTimeout(resolve, 350)); } await rest.put(Routes.applicationCommands(readyClient.user.id), { body: commands.map(command => command.data.toJSON()) }); console.log("Commands refreshed and loaded!"); } catch (error) { console.error("Failed to refresh commands:", error); } });
client.login(token);
